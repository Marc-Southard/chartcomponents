...
//    spatial.drawChart(chartPanel, gr);
    ...
    if (tellme)
    {
      ...
      double heading = Math.toDegrees(gc.getRhumbLineRoute());
      System.out.println("1-2: heading " + heading + ", dist:" + dist);
      gc.setStart(drTwo);
      gc.setArrival(drThree);
      gc.calculateRhumLine();
      dist = gc.getDistance() * 60.0;
      heading = Math.toDegrees(gc.getRhumbLineRoute());
    ...
    GeoPoint dr = GreatCircle.dr(new GeoPoint(Math.toRadians(drOne.getL()), Math.toRadians(drOne.getG())), 25.9, 264);
    GeoPoint forecastOne = new GeoPoint(Math.toDegrees(dr.getL()), Math.toDegrees(dr.getG()));
    java.awt.Point gpfOne = chartPanel.getPanelPoint(forecastOne.getL(), forecastOne.getG());
    ...
    dr = GreatCircle.dr(new GeoPoint(Math.toRadians(drTwo.getL()), Math.toRadians(drTwo.getG())), 33.6, 261);
    GeoPoint forecastTwo = new GeoPoint(Math.toDegrees(dr.getL()), Math.toDegrees(dr.getG()));
    java.awt.Point gpfTwo = chartPanel.getPanelPoint(forecastTwo.getL(), forecastTwo.getG());
    chartPanel.plotLOP(gr, forecastTwo, 102D, 0.571, "Sun");
    chartPanel.postit(gr, "Log:123.48", gpfTwo.x, gpfTwo.y, Color.yellow);
    // First again
    dr = GreatCircle.dr(new GeoPoint(Math.toRadians(forecastOne.getL()), Math.toRadians(forecastOne.getG())), 33.6, 261);
    GeoPoint forecastOneTwo = new GeoPoint(Math.toDegrees(dr.getL()), Math.toDegrees(dr.getG()));
    java.awt.Point gpfOneTwo = chartPanel.getPanelPoint(forecastOneTwo.getL(), forecastOneTwo.getG());
