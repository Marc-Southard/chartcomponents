       ...
       // System.out.println("Starting video for " + al.size() + " points, at " + videoStart);
          for (int i=videoStart; i>=0 && i<al.size() && playVideo; i += videoIncrement)
          {
            if (!pauseVideo)
            {
              currentVideoIndex = i;
              repaint();
              try { Thread.sleep(videoSleep); } catch (InterruptedException ignore) {}
            }
            else
            {
...
//        System.out.println("Video " + (pauseVideo?"paused":"completed"));
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (dragged && lineToDraw != null)
    {
      ...
      String two = "Rhumbline:" + (Integer.toString((int)gc.getRhumbLineDistance())) + "', " + (Integer.toString((int)Math.toDegrees(gc.getRhumbLineRoute())) + "°T");
...
//  }
  ...
  //                   "Bearing: "   + Integer.toString((int)Math.toDegrees(gc.getRhumbLineRoute())) + "°T");
            if (dp == null)
              dp = new DistancePanel();
            dp.setGcDist(dist);
            dp.setRlDist(gc.getRhumbLineDistance());
            
            dp.setFrom(from.toString());
            dp.setTo(to.toString());
            dp.setGCValue(Integer.toString((int)dist) + " nm");
            dp.setRhumblineValue(Integer.toString((int)gc.getRhumbLineDistance()) + " nm");
            dp.setBearingValue(Integer.toString((int)Math.toDegrees(gc.getRhumbLineRoute())) + "°T");
